...
def load_config() -> dict:
    ...
    if not CONFIG_FILE.exists():
        ...
        print(f"❌ 未找到配置文件，已创建示例: {CONFIG_FILE}")
...
async def crawl(config: dict, reset: bool = False):
    ...
    async with aiohttp.ClientSession(headers=headers) as s:
        ...
        while True:
                ...
                try:
                    async with s.get(url, params=params) as r:
                        if r.status == 200:
                            ...
                            print(f"⚠️ 请求被限流（412），等待 {wait:.1f}s 后重试...")
                            await asyncio.sleep(wait)
                        else:
                            print(f"❌ HTTP 状态码: {r.status}")
                            break
                except Exception as e:
                    wait = RETRY_DELAY * (2 ** attempt)
                    print(f"⚠️ 请求异常: {e}，等待 {wait:.1f}s 后重试...")
                    await asyncio.sleep(wait)
            ...
            if js.get("code") != 0:
                print(f"❌ API 返回错误: {js.get('message')}")
                break
